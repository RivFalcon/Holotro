...
Holo.Relic_Gacha{ -- Hololive
    ...
    use = function(self, card, area, copier)
        local pool_modes = {'Synergy','All Stars'}
        for _,J in ipairs(G.jokers.cards)do
            if J.config.center.rarity == 'hololive_Relic' then
                pool_modes[#pool_modes+1] = 'Genmates'
                break
            end
        end
        local hbd_member = Holo.birthday_chart[os.date('%m%d')]
        if hbd_member and not next(find_joker('j_hololive_Relic_'..hbd_member)) then
            pool_modes[#pool_modes+1] = 'Birthday'
        end
        local pool_mode = pseudorandom_element(pool_modes, pseudoseed('RelicGachaMode'))
        local _pool = {}
        ...
        if pool_mode == 'Synergy' then
            ...
            local hand_usage = G.GAME.hand_usage
            hand_usage["Five of a Kind"] = hand_usage["Flush Five"] or hand_usage["Five of a Kind"]
            hand_usage["Four of a Kind"] = hand_usage["Five of a Kind"] or hand_usage["Four of a Kind"]
            hand_usage["Full House"] = hand_usage["Flush House"] or hand_usage["Full House"]
            hand_usage["Flush"] = hand_usage["Flush Five"] or hand_usage["Flush House"] or hand_usage["Straight Flush"] or hand_usage["Flush"]
            hand_usage["Straight"] = hand_usage["Straight Flush"] or hand_usage["Straight"]
            hand_usage["Three of a Kind"] = hand_usage["Four of a Kind"] or hand_usage["Full House"] or hand_usage["Three of a Kind"]
            hand_usage["Two Pair"] = hand_usage["Full House"] or hand_usage["Two Pair"]
            hand_usage["Pair"] = hand_usage["Three of a Kind"] or hand_usage["Two Pair"] or hand_usage["Pair"]
            ...
            local syn_table = {
                ...
                Bae      = true,
                Laplus   = false, -- deck_stat.X or c_usage.c_planet_x,
                Lui      = false, -- deck_stat.X or c_usage.c_hermit,
                Koyori   = false, -- deck_stat.X,
                Chloe    = false, -- deck_stat.X,
                Iroha    = false, -- deck_stat.X,
                Zeta     = false,
        ...
        if pool_mode == 'All Stars' then
            ...
            if next(find_joker('Showman')) then
                _pool = implemented_relics
            else
                for _,m in ipairs(implemented_relics) do
                    if not next(find_joker('j_hololive_Relic_'..m)) then
                        _pool[#_pool+1] = m
                    end
                end
